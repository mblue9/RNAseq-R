...
You can see the slides covering RNA-seq basics [here](slides-01-rnaseq-basics.html).

**Acknowledgements**

This material was inspired by this [case study]((http://www.statsci.org/smyth/pubs/QLedgeRPreprint.pdf)) by [@Lun2016] and quality control steps from this [material](http://monashbioinformaticsplatform.github.io/RNAseq-DE-analysis-with-R/RNAseq_DE_analysis_with_R.html).
...
The data for this tutorial comes from a paper by [@Fu2015]. Both the raw data (sequence reads) and processed data (counts) can be downloaded from Gene Expression Omnibus database (GEO) under accession number [GSE60450](http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE60450). This study examines the expression profiles of basal stem-cell enriched cells (B) and committed luminal cells (L) in the mammary gland of virgin, pregnant and lactating mice. Six groups are present, with one for each combination of cell type and mouse status. Each group contains two biological replicates.
We will first use the counts file as a starting point for our analysis. This data has already been aligned to the mouse genome. The command line tool featureCounts [@Liao2014] was used to count reads mapped to mouse genes from Refseq annotation (see [@Fu2015] for details).
...
You can try out the code, without needing to install anything, through the interactive challenges option. To access the file containing all challenges, right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenges_all/)

<!---------------->

## Data files

The RNA-Seq data files used is this workshop are available from [Figshare](https://figshare.com/s/1d788fd384d33e913a2a). They were originally obtained from [GSE60450](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE60450). 
We will show you how to import the files into R from the links. Alternatively you could download them and place them in your working directory, for example in a folder called `data` and then refer to them in your code as e.g  `"data/GSE60450_Lactation-GenewiseCounts.txt"`).

Data files:

* [GSE60450_Lactation-GenewiseCounts.txt](https://ndownloader.figshare.com/files/5057929?private_link=1d788fd384d33e913a2a)
* [SampleInfo.txt](https://ndownloader.figshare.com/files/5999829?private_link=1d788fd384d33e913a2a)
* [SampleInfo_Corrected.txt](https://ndownloader.figshare.com/files/5999832?private_link=1d788fd384d33e913a2a)
* [Mm.c2.all.v7.1.entrez.rds](http://bioinf.wehi.edu.au/MSigDB/v7.1/Mm.c2.all.v7.1.entrez.rds)  
* [Mm.h.all.v7.1.entrez.rds](http://bioinf.wehi.edu.au/MSigDB/v7.1/Mm.h.all.v7.1.entrez.rds)
...
BiocManager::install(c("limma", "edgeR", "Glimma", "org.Mm.eg.db", "BiasedUrn", "GO.db", "gplots", "RColorBrewer", "NMF"))
```
...
Then we'll import the counts and sample information files into R.
...
sampleinfo <- read.delim("https://ndownloader.figshare.com/files/5999829?private_link=1d788fd384d33e913a2a", stringsAsFactors = TRUE)
```
...
You can see the slides covering filtering and quality control [here](slides-02-rnaseq-filtering-qc.html).
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 1**
...
Try this challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge1/)

<!---------------->

<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r, class.output="solution", echo=FALSE}
...
<details>
<summary>More details: labelling</summary>
```{r barplot-labelled}
...
<div id="challenge">
<i class="far fa-question-circle"></i> **Discussion**
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 2**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge2/)

<!---------------->

<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r MDSplot-nicer, class.output="solution", echo=FALSE}
# Solution to 3
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 3**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge3/)
...
<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r heatmap-solution, class.output="solution", echo=FALSE}
# Solution 1 and 2
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 4**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge4/)
...
<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r MDplots-solution, class.output="solution", echo=FALSE, fig.height=5, fig.width=10}
par(mfrow=c(1,2))
plotMD(logcounts,column = 11)
abline(h=0,col="grey")
plotMD(y,column = 11)
...
You can see the slides covering differential expression analysis [here](slides-03-differential-expression-analysis.html).

Now that we are happy that we have normalised the data and that the quality looks good, we can continue to testing for differentially expressed genes. There are a number of packages to analyse RNA-Seq data. The *limma* package [@Ritchie2015] offers the `voom` function, which transforms the read counts into logCPMs while taking into account the mean-variance relationship in the data [@Law2014]. After vooming, users can apply a linear model to the voom transformed data to test for differentially expressed genes, using standard *limma* commands.
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 5**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge5/)

<!---------------->

<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r vennDiagram, class.output="solution", echo=FALSE}
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 6**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge6/)

<!---------------->

<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r plots-postDE-solution, class.output="solution", echo=FALSE, fig.height=5, fig.width=10}
par(mfrow=c(1,2))
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 7**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge7/)

<!---------------->

<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r stripcharts-solution, class.output="solution", echo=FALSE}
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 8**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge8/)
...
You can see the slides covering gene set testing [here](slides-04-gene-set-testing.html).
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 9**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge9/)
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 10**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge10/)
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 11**
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge11/)
...
<div id="challenge">
<i class="fas fa-pencil-alt"></i> **Challenge 12**
...
3. You can put two gene sets on one plot, for example a set that is up-regulated and one that is down-regulated, by adding a gene set to the index2 argument. Produce a barcodeplot with the two sets REACTOME_REPRESSION_OF_WNT_TARGET_GENES and PID_WNT_CANONICAL_PATHWAY.
</div>
...
Try the challenge interactively. Right-click on the launch button below and open in a new tab. May take a minute to launch.  
[![Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/mblue9/RNAseq-R_learnr_binder/HEAD?urlpath=shiny/challenge12/)
...
<details>
<summary> <i class="far fa-eye"></i> **Solution**</summary>
```{r barcodeplots-solution, class.output="solution", echo=FALSE}
# Solution 1
